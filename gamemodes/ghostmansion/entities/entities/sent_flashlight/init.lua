...
function ENT:CreateLight( col, bright, size, len )
	...
	self.FlashlightEnt:SetKeyValue( "nearz", 100 ) 
	self.FlashlightEnt:SetKeyValue( "lightfov", size ) 
	
	self.FlashlightEnt.Color = col
	self.FlashlightEnt.Bright = bright
	self.FlashlightEnt.Size = size
	self.FlashlightEnt.Length = len
...
function ENT:PhysicsSimulate( phys, deltatime )
	...
	self.ShadowParams.pos = self.Entity:GetOwner():GetShootPos() - self.Entity:GetOwner():GetAimVector()*20
	self.ShadowParams.angle = self.Entity:GetOwner():GetAimVector():Angle() + Angle(20,0,0)
	self.ShadowParams.maxangular = 800000 
	self.ShadowParams.maxangulardamp = 900000
	self.ShadowParams.maxspeed = 800000 
	self.ShadowParams.maxspeeddamp = 900000
	self.ShadowParams.dampfactor = 0.9999
